...
CREATE OR REPLACE VIEW dbo.VW_ALL_EMPLOYEES AS
    ...
    SELECT en.EMPLOYEE_NUMBER EMPLOYEE, FIRST_NAME, MIDDLE_NAME, LAST_NAME,
           EMP_PHONE_AREA, EMP_PHONE_NUMBER,
		   EMP_ADDRESS_STNUM, EMP_ADDRESS_STNAME, 
...
CREATE OR REPLACE PROCEDURE dbo.SP_EMP_GET IS
	W_EMPLOYEE		NUMBER(6);
	W_FIRST_NAME	    VARCHAR2(10);
	W_MIDDLE_NAME       VARCHAR2(10);
	W_LAST_NAME         VARCHAR2(20);
	W_PHONE_AREA		VARCHAR2(3);
	W_PHONE_NUMBER      VARCHAR2(8);
	W_EMP_STREET_NUM   	VARCHAR2(5);
	W_EMP_STREET_NAME	VARCHAR2(20);
	W_EMP_CITY      	VARCHAR2(20);
	W_EMP_STATE     	VARCHAR2(2);
	W_EMP_ZIP		NUMBER(5);
	
CURSOR EMP_CURSOR IS 
     SELECT EMPLOYEE, FIRST_NAME, MIDDLE_NAME, LAST_NAME, EMP_PHONE_AREA, EMP_PHONE_NUMBER, EMP_ADDRESS_STNUM, EMP_ADDRESS_STNAME, CITY, STATE, ZIP
...
OPEN EMP_CURSOR;
LOOP
   FETCH EMP_CURSOR INTO W_EMPLOYEE, W_FIRST_NAME, W_MIDDLE_NAME, W_LAST_NAME, W_PHONE_AREA, W_PHONE_NUMBER, W_EMP_STREET_NUM, W_EMP_STREET_NAME, W_EMP_CITY, W_EMP_STATE, W_EMP_ZIP;
   EXIT WHEN EMP_CURSOR%NOTFOUND;
   DBMS_OUTPUT.PUT_LINE(W_EMPLOYEE || W_FIRST_NAME || W_MIDDLE_NAME || W_LAST_NAME || W_PHONE_AREA || W_PHONE_NUMBER || W_EMP_STREET_NUM || W_EMP_STREET_NAME || W_EMP_CITY || W_EMP_STATE || W_EMP_ZIP);
END LOOP;
	DBMS_OUTPUT.PUT_LINE('======================================');
